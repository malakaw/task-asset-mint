...
#_(defn  calculate_mini_fee [tx-raw-body-file tx-in-count tx-out-count]
  (assoc command_calculate-min-fee 4  tx-raw-body-file 6 tx-in-count 8 tx-out-count 10 "/home/ubuntu/install/temp/protocol.json" )
  )
  ...
  (let [
    ...
    (let [result_ (try
                    (count
                     (get
                      (client/get url_
                                  {:headers {:project_id "testnetSYxxxx"}})
                      :body
                      ))
